...
* Client make a reservation two hours before the ride **Here as domain or do we have to do the requirements into G7? [Gilles : I would say requirements, TM: requirements]**
...
* Taxi request: it is the request the system sends (automatically or after a client request) to taxi to create a ride, specifying start point, client and other elements if they are available. **KEEP or remove?[TM: better to use notification instead of request]**
...
* The registration/deletion by company of a taxi driver is done in the same way of the old system, so we do not have to do this part. **keep or remove [gilles : keep, TM: keep => "it's done on the old system" we just interface to the old one.]**
